...
class PerpBalanceMapping(BaseModel):
    ...
    def total_position_value(self) -> float:
        """
        모든 포지션의 명목(순) 크기의 합을 반환합니다.
        """
        total_value = 0.0
        for pos in self.position.oneWay.values():
            total_value += pos.positionValue
        for pos in self.position.twoWay.values():
            total_value += pos.positionValue
        return total_value
